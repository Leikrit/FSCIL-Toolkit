...
class CIFAR10(VisionDataset):
    ...
    def SelectfromDefault(self, data, targets, index):
        ...
        for i in index:
            ...
            if data_tmp is None:
                ...
                # 确保data_tmp和data[ind_cl]的形状兼容
                if data_tmp.shape[1:] != data[ind_cl].shape[1:]:
                    raise ValueError("Incompatible shapes for data_tmp and data[ind_cl]")
                # 确保targets_tmp和targets[ind_cl]的形状兼容
                if len(targets_tmp.shape) > 1 and targets_tmp.shape[1:] != targets[ind_cl].shape[1:]:
                    raise ValueError("Incompatible shapes for targets_tmp and targets[ind_cl]")
                data_tmp = np.vstack((data_tmp, data[ind_cl]))
                targets_tmp = np.hstack((targets_tmp, targets[ind_cl]))
    ...
    def NewClassSelector(self, data, targets, index):
        ...
        index = ind_np.reshape((5, 5))
        for i in index:
            ind_cl = i.flatten()  # 确保ind_cl是一维数组
            if data_tmp is None:
                data_tmp = data[ind_cl]
                targets_tmp = targets[ind_cl]
            else:
...
class CIFAR_concate(VisionDataset):
