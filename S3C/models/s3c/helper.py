...
def save_features(model, testloader, args, session):
    ...
    with torch.no_grad():
        for i, batch in enumerate(testloader):
            ...
            # data = data.view(-1, 3, 32, 32)  # 确保这里的尺寸是正确的
            # data = data.view(-1, 3, 32, 32)
            data = data.view(-1, 3, 224, 224)  # 确保这里的尺寸是正确的
            # data = data.view(-1, 3, 84, 84)
            logits, features, _ = model(data, stochastic=False)
            
            # 确保features的批次大小与其他张量一致
            features = features.view(-1, 4, features.size(-1))  # 调整features的形状
            features = features[:, 0, :]  # 选择第一个旋转的特征，或者根据需要选择
            ...
            labels.append(test_label[::4])  # 确保labels的批次大小与features一致
    ...
    # 使用torch.cat代替torch.stack
    data_features = torch.cat(data_features).view(-1, 64)  # 确保所有特征张量在连接维度上的形状相同
    data_features = F.normalize(data_features, p=2, dim=-1)
    labels = torch.cat(labels).view(-1, 1)
    predictions = torch.cat(predictions).view(-1, 1)
...
#     data_features = torch.cat(data_features).view(-1, 64)  # 使用torch.cat代替torch.stack
#     data_features = F.normalize(data_features, p=2, dim=-1)
#     labels = torch.cat(labels).view(-1, 1)
#     predictions = torch.cat(predictions).view(-1, 1)
